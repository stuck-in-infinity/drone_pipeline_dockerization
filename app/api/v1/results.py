...
def download_kmz(project=Depends(get_project)):
    f = os.path.join(project_paths(project.id)["step4_output"], "species_map.kmz")
...
def download_master(project=Depends(get_project)):
    f = os.path.join(project_paths(project.id)["step2_output"], "crown_master.csv")
    if not os.path.exists(f):
...
def download_polyspecies(project=Depends(get_project)):
    f = os.path.join(project_paths(project.id)["step2_output"], "polygon_species.csv")
    if not os.path.exists(f):
...
def download_cm(project=Depends(get_project)):
    f = os.path.join(project_paths(project.id)["step3_output"], "confusion_matrix.png")
    if not os.path.exists(f):
